...
function updatePage(data) {
  ...
  data.forEach(function (item) {
    ...
    clonedContainer.querySelector(
      ".username a"
    ).href = `/profileUser?userId=${item.author_id}`;
    if (item.Avatar.length < 6) {
      clonedContainer.querySelector(".post__avatar").src =
        "/assets/img/avatar.svg";
    } else {
      clonedContainer.querySelector(".post__avatar").src = item.Avatar;
    }
    clonedContainer.querySelector(".timestamp").textContent =
    ...
    clonedContainer.querySelector(".commentCount").textContent =
      "Комментарии: " + item.comment_count;
    clonedContainer.querySelector(
      ".commentCount"
    ).href = `/h/post/${item.Id}/comments`;
